...
INFO "fly -t \"$TARGET\" set-pipeline -n -p \"$PIPELINE\" -c \"$DIRECTORY\"/concourse.yml $LOAD_VARS"
fly -t "$TARGET" set-pipeline -n -p "$PIPELINE" -c "$DIRECTORY"/concourse.yml $LOAD_VARS
